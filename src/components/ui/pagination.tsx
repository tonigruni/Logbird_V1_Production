...
function PaginationPrev({ className, ...props }: React.ButtonHTMLAttributes<HTMLButtonElement>) {
  return (
    <button
      ...
      className={cn(
        'flex h-10 items-center gap-1.5 px-4 text-sm font-semibold text-[#5a6061] hover:bg-[#f2f4f4] hover:text-[#1F3649] transition-colors disabled:opacity-40 disabled:cursor-not-allowed cursor-pointer',
        className,
      )}
      style={{ borderRadius: 10 }}
      {...props}
