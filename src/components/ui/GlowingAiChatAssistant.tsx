    ...
    <div className="fixed bottom-6 right-6 z-50">
      ...
      {isChatOpen && (
        <div
          ...
          style={{ bottom: '4.5rem' }}
        >
          <div className="flex flex-col bg-white border border-[#ebeeef] shadow-[0_16px_48px_rgba(12,22,41,0.12)] overflow-hidden" style={{ borderRadius: 15 }}>

            {/* Header */}
            <div className="flex items-center justify-between px-5 py-3.5 border-b border-[#f2f4f4] bg-[#f2f4f4]">
              <div className="flex items-center gap-2">
                <div className="w-2 h-2 rounded-full bg-[#4ade80] animate-pulse" />
                <span className="text-xs font-semibold text-[#1F3649]">Journal AI</span>
              </div>
              <button onClick={() => setIsChatOpen(false)} className="p-1 hover:bg-[#f2f4f4] transition-colors" style={{ borderRadius: 8 }}>
                <X size={14} className="text-[#5a6061]" />
              </button>
            </div>
                  ...
                  <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                    ...
                    <div className={`max-w-[80%] px-3 py-2 rounded-2xl text-xs leading-relaxed ${
                      msg.role === 'user'
                        ...
                        : 'bg-white border border-[#f2f4f4] text-[#1F3649] rounded-bl-sm shadow-sm'
                    }`}>
                      {msg.content}
                    </div>
                  </div>
            ...
            <div className="px-4 pb-4 pt-3">
              <textarea
                ...
                className="w-full bg-[#f2f4f4] border border-transparent rounded-[15px] px-4 py-3 text-sm text-[#1F3649] placeholder:text-[#adb3b4] resize-none outline-none focus:border-[#1F3649]/20 focus:bg-white focus:ring-2 focus:ring-[#1F3649]/10 transition-all scrollbar-hide leading-relaxed"
