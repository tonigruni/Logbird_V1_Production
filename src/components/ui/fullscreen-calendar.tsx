    ...
    <div className="flex flex-1 flex-col">
      ...
      <div className="flex flex-col gap-4 p-4 md:flex-row md:items-center md:justify-between md:gap-0 lg:flex-none border-b border-[#f2f4f4]">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-base font-black text-[#1F3649] tracking-tight">{format(firstDayCurrentMonth, 'MMMM, yyyy')}</h2>
        ...
        <div className="flex items-center gap-2">
          <div className="inline-flex -space-x-px rounded-[15px] overflow-hidden border border-[#ebeeef]">
            <Button onClick={previousMonth} variant="outline" size="icon" className="rounded-none border-0 border-r border-[#ebeeef]">
              <ChevronLeft size={15} />
            </Button>
            <Button onClick={goToToday} variant="outline" className="rounded-none border-0 px-4 text-xs">
              Today
            </Button>
            <Button onClick={nextMonth} variant="outline" size="icon" className="rounded-none border-0 border-l border-[#ebeeef]">
                ...
                <header className="flex items-center justify-between p-2">
                  <button
                    ...
                    className={cn(
                      ...
                      !isEqual(day, selectedDay) && !isToday(day) && isSameMonth(day, firstDayCurrentMonth) && 'text-[#1F3649] hover:bg-[#f2f4f4]',
              ...
              >
                <time
                  ...
                  className={cn(
                    ...
                    isToday(day) && !isEqual(day, selectedDay) && 'bg-[#1F3649]/10 text-[#1F3649]',
                    !isEqual(day, selectedDay) && !isToday(day) && isSameMonth(day, firstDayCurrentMonth) && 'text-[#1F3649]',
                    !isEqual(day, selectedDay) && !isToday(day) && !isSameMonth(day, firstDayCurrentMonth) && 'text-[#adb3b4]',
                  )}
                >
                  {format(day, 'd')}
