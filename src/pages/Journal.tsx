...
const CATEGORY_COLORS: Record<string, { bg: string; text: string; dot: string }> = {
  ...
  Work:      { bg: 'bg-[#5a6061]/10',  text: 'text-[#5a6061]',  dot: '#5a6061' },
...
function getCatColor(cat: string) {
  return CATEGORY_COLORS[cat] ?? { bg: 'bg-[#f2f4f4]', text: 'text-[#5a6061]', dot: '#adb3b4' }
}
...
const MOOD_META: Record<number, { label: string; short: string; chipClass: string; color: string; bg: string; icon: LucideIcon }> = {
  ...
  3: { label: 'Neutral',  short: 'Neutral',  chipClass: 'bg-[#f2f4f4] text-[#5a6061]',     color: '#9ca3af', bg: '#f9fafb', icon: Meh   },
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
                ...
                >
                  ...
                  <div className="flex items-center gap-5 mt-6">
                    <button onClick={() => applyBuiltIn(featuredTpl)}
                      className="bg-white hover:bg-white/90 text-[#1F3649] text-sm font-semibold px-6 py-2.5 rounded-[10px] transition-all cursor-pointer">
                      Apply Template
                    </button>
              ...
              <div className="col-span-1 md:col-span-4 bg-[#f2f4f4] card p-6 md:p-8 flex flex-col justify-between min-h-[200px] md:min-h-[280px] !border-2 !border-dashed !border-[#adb3b4]/30 hover:!border-[#1F3649]/30 transition-colors group">
                <div>
                  <div className="w-12 h-12 rounded-full bg-white shadow-sm flex items-center justify-center mb-4 group-hover:scale-105 transition-transform"><Plus size={20} className="text-[#5a6061]"/></div>
                  <h3 className="text-base font-bold text-[#1F3649] mb-1.5">Blank canvas</h3>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {[
                ...
                <button key={id} onClick={() => gotoView(id as Exclude<View, 'editor'>)}
                  ...
                  <div className="text-2xl font-extrabold text-[#1F3649] mb-1">{value}</div>
                  <div className="text-sm font-semibold text-[#1F3649] mb-0.5">{label}</div>
                ...
                {entries.slice(0,3).map(entry => {
                  ...
                  return (
                    <div key={entry.id} onClick={() => { openEntry(entry); setView('editor') }}
                      ...
                      <h3 className="text-sm font-bold text-[#1F3649] mb-2 line-clamp-2 group-hover:text-[#1F3649] transition-colors">{entry.title}</h3>
        ...
        <div className="flex flex-col lg:flex-row gap-6 items-start animate-fade-in">
                  ...
                  return (
                    <div key={entry.id} className="bg-white card p-5 hover:shadow-[0_8px_30px_rgba(12,22,41,0.08)] transition-all group flex items-start gap-4">
                      <div className="shrink-0 text-center w-10 cursor-pointer pt-0.5" onClick={() => { openEntry(entry); setView('editor') }}>
                        <div className="text-lg font-extrabold text-[#1F3649] leading-none">{format(new Date(entry.created_at),'d')}</div>
                      ...
                      <div className="flex-1 min-w-0 cursor-pointer" onClick={() => { openEntry(entry); setView('editor') }}>
                        <div className="flex items-start justify-between gap-3 mb-1.5">
                          <h3 className="text-sm font-bold text-[#1F3649] truncate group-hover:text-[#1F3649] transition-colors">{entry.title}</h3>
                      ...
                      <div className="flex flex-col items-center gap-2 shrink-0">
                        <button
                          onClick={(e) => { e.stopPropagation(); updateEntry(entry.id, { is_favorite: !entry.is_favorite }) }}
                          className={cn('p-1.5 rounded-lg transition-all cursor-pointer', entry.is_favorite ? 'text-[#ca8a04]' : 'text-[#ebeeef] hover:text-[#ca8a04]')}
                        >
          ...
          <div className="w-full lg:w-72 lg:shrink-0 space-y-4">
            ...
            <div className="bg-white card p-5">
              <div className="flex items-center gap-2 mb-4"><BarChart2 size={14} className="text-[#1F3649]"/><h3 className="text-xs font-bold text-[#1F3649] uppercase tracking-wider">Quick Stats</h3></div>
              <div className="grid grid-cols-2 gap-2.5">
                {[
                  ...
                  <div key={label} className="bg-[#f2f4f4] rounded-[10px] p-3 text-center">
                    <div className="text-lg font-extrabold text-[#1F3649]">{value}</div>
            ...
            {/* Mood Distribution */}
            <div className="bg-white card p-5">
              <div className="flex items-center gap-2 mb-4"><TrendingUp size={14} className="text-[#1F3649]"/><h3 className="text-xs font-bold text-[#1F3649] uppercase tracking-wider">Mood Distribution</h3></div>
              {(() => {
                  ...
                  <>
                    <div className="flex items-center gap-3">
                      <svg width={96} height={96} viewBox="0 0 100 100" className="shrink-0">
                        ...
                        <text x={50} y={46} textAnchor="middle" fill="#1F3649" fontSize={15} fontWeight={700}>{total}</text>
                        ...
                        {([5,4,3,2,1] as const).map(k => {
                          ...
                          return (
                            <button key={k} onClick={() => setMoodFilter(isActive ? null : {min:k,max:k})}
                              ...
                              <span className="flex-1 text-left text-[#1F3649] font-medium">{meta.label}</span>
            ...
            {categoryCounts.length > 0 && (
              <div className="bg-white card p-5">
                <div className="flex items-center gap-2 mb-4"><Tag size={14} className="text-[#1F3649]"/><h3 className="text-xs font-bold text-[#1F3649] uppercase tracking-wider">Categories</h3></div>
            ...
            {/* Common Themes */}
            <div className="bg-white card p-5">
              <div className="flex items-center gap-2 mb-4"><Hash size={14} className="text-[#1F3649]"/><h3 className="text-xs font-bold text-[#1F3649] uppercase tracking-wider">Common Themes</h3></div>
                ...
                {popularWords.length === 0 ? (
                  ...
                  <button key={word} onClick={() => setSearchFilter(searchFilter===word?'':word)}
                    className={cn('px-2.5 py-1 text-xs font-medium rounded-[15px] transition-all cursor-pointer',
                      searchFilter===word ? 'bg-[#1F3649] text-white' : 'bg-[#f2f4f4] text-[#5a6061] hover:bg-[#f2f4f4]')}>
            ...
            <div className="mb-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
                ...
                <div className="col-span-1 md:col-span-7 bg-white card p-6">
                  <h3 className="text-xs font-bold text-[#1F3649] uppercase tracking-wider mb-3">How it works</h3>
                  <p className="text-sm text-[#1F3649] leading-relaxed mb-4">{activeTemplateDetail.howItWorks}</p>
                  <div className="grid grid-cols-2 gap-3">
                    <div className="bg-[#f2f4f4] rounded-[10px] p-3">
                      ...
                      <div className="text-xs font-semibold text-[#1F3649] leading-snug">{activeTemplateDetail.principle}</div>
                    </div>
                    <div className="bg-[#f2f4f4] rounded-[10px] p-3">
                      <div className="text-[10px] font-bold text-[#adb3b4] uppercase tracking-widest mb-1">Focus</div>
                      <div className="text-xs font-semibold text-[#1F3649] leading-snug">{activeTemplateDetail.focus}</div>
                    </div>
                  </div>
                </div>
                {/* Why use this */}
                <div className="col-span-1 md:col-span-5 bg-[#f2f4f4] card p-6 !border-2 !border-dashed !border-[#adb3b4]/30">
                  <h3 className="text-xs font-bold text-[#1F3649] uppercase tracking-wider mb-3">Why use this</h3>
                  <ul className="space-y-2 mb-4">
                    {activeTemplateDetail.benefits.map((b, i) => (
                      <li key={i} className="flex items-start gap-2 text-sm text-[#1F3649] leading-snug">
              ...
              <div className="flex items-center gap-4 mt-7">
                ...
                <span className="text-[11px] font-bold text-[#5a6061] uppercase tracking-widest">Your Entry</span>
                <div className="flex-1 h-px bg-[#ebeeef]"/>
              </div>
            </div>
          )}
              ...
              <div className="mb-8">
                ...
                <div className="flex items-start gap-3">
                  <input
                    ...
                    className="flex-1 min-w-0 bg-transparent text-2xl md:text-3xl font-extrabold text-[#1F3649] placeholder:text-[#adb3b4] focus:outline-none tracking-tight leading-tight"
                  />
                  {/* Templates dropdown */}
                  <DropdownMenu.Root>
                    <DropdownMenu.Trigger asChild>
                      <button className="flex items-center gap-2 px-3 py-2 mt-1 rounded-[12px] hover:bg-[#f2f4f4] transition-all cursor-pointer text-sm font-semibold text-[#5a6061] shrink-0 border border-[#ebeeef]">
                    ...
                    <DropdownMenu.Portal>
                      <DropdownMenu.Content
                        ...
                        side="bottom"
                        sideOffset={6}
                        className="z-50 min-w-[260px] rounded-[15px] border border-[#ebeeef] bg-white shadow-[0_12px_44px_rgba(12,22,41,0.10)] p-1.5 outline-none"
                  ...
                  {activeTemplateDetail.prompts.map((p, i) => (
                    <div key={i} className="group">
                      ...
                      <div className="flex items-center gap-2 mb-3">
                        ...
                        <label className="text-[11px] font-bold text-[#5a6061] uppercase tracking-widest">
                          {p.label}
                        </label>
                      </div>
                      {/* Textarea */}
                      <textarea
                        ...
                        rows={4}
                        className="w-full rounded-[15px] border border-[#ebeeef] bg-white px-5 py-4 text-sm text-[#1F3649] shadow-sm shadow-black/5 transition-shadow placeholder:text-[#5a6061]/50 focus-visible:border-[#1F3649]/30 focus-visible:outline-none focus-visible:ring-[3px] focus-visible:ring-[#1F3649]/10 resize-none leading-relaxed"
                      />
                    </div>
                  ))}
                  {/* Extra thoughts */}
            ...
            <aside className="w-full lg:w-72 lg:shrink-0 lg:sticky lg:top-6 space-y-0 lg:overflow-y-auto lg:max-h-[calc(100vh-6rem)]" style={{ scrollbarWidth: 'none' }}>
              <div className="bg-white card overflow-hidden">
                ...
                <div className="p-6 border-b border-[#f2f4f4]">
                  <div className="flex items-center justify-between mb-4">
                    <h3 className="text-xs font-bold text-[#1F3649] uppercase tracking-wider">Current Mood</h3>
                  </div>
                ...
                <div className="p-6 border-b border-[#f2f4f4]">
                  <div className="flex items-center justify-between mb-4">
                    <h3 className="text-xs font-bold text-[#1F3649] uppercase tracking-wider">Category</h3>
                      ...
                      return (
                        <button key={cat} onClick={() => setEntryCategory(active ? entryCategory.filter(c => c !== cat) : [...entryCategory, cat])}
                          className={cn('px-3 py-1.5 rounded-[15px] text-xs font-semibold transition-all cursor-pointer',
                            active ? `${cc.bg} ${cc.text} shadow-sm` : 'bg-[#f2f4f4] text-[#5a6061] hover:bg-[#f2f4f4]')}>
                          {cat}
                        </button>
                      )
                    })}
                ...
                <div className="p-6 border-b border-[#f2f4f4]">
                  ...
                  <div className="space-y-3">
                    <div>
                      ...
                      <div className="flex items-center gap-2.5 rounded-[15px] border border-[#ebeeef] bg-white px-3 py-2.5 shadow-sm shadow-black/5 transition-shadow focus-within:border-[#1F3649]/30 focus-within:ring-[3px] focus-within:ring-[#1F3649]/10">
                        ...
                        />
                        {entryLocation && <button onClick={() => setEntryLocation('')} className="text-[#adb3b4] hover:text-[#5a6061] cursor-pointer"><X size={12}/></button>}
                      </div>
                    </div>
                    <div>
                      ...
                      <div className="flex items-center gap-2.5 rounded-[15px] border border-[#ebeeef] bg-white px-3 py-2.5 shadow-sm shadow-black/5 transition-shadow focus-within:border-[#1F3649]/30 focus-within:ring-[3px] focus-within:ring-[#1F3649]/10">
                        ...
                        <input
                          ...
                          className="flex-1 bg-transparent text-sm text-[#1F3649] placeholder:text-[#adb3b4] focus:outline-none min-w-0"
                        />
                        {entryWeather && <button onClick={() => setEntryWeather('')} className="text-[#adb3b4] hover:text-[#5a6061] cursor-pointer"><X size={12}/></button>}
                      </div>
                    </div>
                  </div>
                </div>
                          ...
                          <button key={v} onClick={() => setSleepQuality(sleepQuality === v ? null : v)}
                            className={cn('flex-1 h-7 rounded-lg text-xs font-bold transition-all cursor-pointer border',
                              sleepQuality === v
                                ...
                                : 'bg-[#f2f4f4] text-[#5a6061] border-transparent hover:bg-[#f2f4f4]')}>
                            {v}
                          </button>
                        ))}
                      </div>
            ...
            <div className="w-full lg:w-[280px] lg:shrink-0 flex flex-col gap-3">
                  ...
                  return (
                    <div key={entry.id} className="bg-white card p-4 hover:shadow-[0_8px_30px_rgba(12,22,41,0.08)] transition-all group cursor-pointer"
                      ...
                      <div className="flex items-start justify-between gap-2 mb-1.5">
                        <h3 className="text-sm font-bold text-[#1F3649] leading-snug group-hover:text-[#1F3649] transition-colors line-clamp-2">{entry.title}</h3>
                        <button onClick={e => { e.stopPropagation(); updateEntry(entry.id, { is_favorite: !entry.is_favorite }) }}
                          className={cn('p-1 rounded-lg shrink-0 transition-all cursor-pointer', entry.is_favorite ? 'text-[#ca8a04]' : 'text-[#ebeeef] hover:text-[#ca8a04]')}>
                          <Star size={12} fill={entry.is_favorite ? 'currentColor' : 'none'}/>
                        </button>
                      </div>
              ...
              <div className="bg-white card p-5">
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2">
                    ...
                    <h3 className="text-xs font-bold text-[#1F3649] uppercase tracking-wider">Entry Rate</h3>
                  </div>
                  <select
                    value={calRatePeriod}
                ...
                <div className="flex items-end gap-1.5">
                  <span className="text-3xl font-extrabold text-[#1F3649] leading-none">{calcRate(calRatePeriod)}</span>
              ...
              <div className="bg-white card p-5">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <h3 className="text-xs font-bold text-[#1F3649] uppercase tracking-wider">Avg Gap</h3>
                </div>
                <div className="flex items-end gap-1.5">
                  <span className="text-3xl font-extrabold text-[#1F3649] leading-none">{avgGapLabel}</span>
      ...
      {view === 'templates' && (
        <div className="animate-fade-in">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
            <div>
              <h2 className="text-xl font-extrabold text-[#1F3649] tracking-tight">Template Library</h2>
              ...
              {(['All', 'Personal', 'Work', 'Gratitude'] as TemplateFilter[]).map(f => (
                <button key={f} onClick={() => setTemplateFilter(f)}
                  className={cn('px-4 py-1.5 text-xs font-semibold rounded-[7px] transition-all cursor-pointer',
                    templateFilter===f ? 'bg-white text-[#1F3649] shadow-sm' : 'text-[#5a6061] hover:text-[#1F3649]')}>
                  {f}
                </button>
              ))}
            </div>
              ...
              return (
                <button onClick={() => applyBuiltIn(tpl)}
                  ...
                  <div className="relative z-10 flex items-start justify-between">
                    ...
                    <ChevronRight size={14} className="text-[#ebeeef] group-hover:text-[#1F3649] transition-colors shrink-0"/>
                  </div>
                  <div className="relative z-10">
                    <span className="text-[10px] font-bold text-[#adb3b4] uppercase tracking-widest block mb-1">{tpl.category}</span>
                    <h3 className="font-extrabold text-[#1F3649] tracking-tight leading-snug mb-1.5">{tpl.name}</h3>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
                      ...
                      <button onClick={() => applyBuiltIn(BUILT_IN_TEMPLATES[0])}
                        ...
                        <div className="relative z-10">
                          <h3 className="text-2xl font-extrabold text-[#1F3649] mb-2 tracking-tight">{BUILT_IN_TEMPLATES[0].name}</h3>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                      ...
                      <button onClick={() => { setNewTemplateName(''); setNewTemplateContent(''); setShowTemplateModal(true) }}
                        ...
                        <div>
                          <h3 className="text-sm font-bold text-[#1F3649] mb-0.5">Custom Template</h3>
              ...
              <div className="grid grid-cols-3 gap-4">
                {templates.map(t => (
                  <div key={t.id} className="bg-white card p-5 hover:shadow-[0_10px_40px_rgba(12,22,41,0.06)] transition-all">
                    ...
                    <h4 className="font-semibold text-[#1F3649] mb-1">{t.name}</h4>
        ...
        <Dialog.Portal>
          ...
          <Dialog.Content className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-50 w-full max-w-2xl max-h-[90vh] bg-white rounded-[20px] shadow-[0_24px_80px_rgba(12,22,41,0.18)] outline-none data-[state=open]:animate-fade-in flex flex-col">
            ...
            <div className="flex items-center justify-between px-7 pt-6 pb-4 border-b border-[#f2f4f4] shrink-0">
              <div>
                <Dialog.Title className="text-base font-bold text-[#1F3649]">Create Custom Template</Dialog.Title>
              ...
              <Dialog.Close asChild>
                <button className="w-7 h-7 flex items-center justify-center rounded-full hover:bg-[#f2f4f4] text-[#adb3b4] hover:text-[#1F3649] transition-colors cursor-pointer">
                  <X size={15} />
                </button>
              </Dialog.Close>
            </div>
            ...
            <div className="flex-1 overflow-y-auto px-7 py-5 space-y-5" style={{ scrollbarWidth: 'none' }}>
              ...
              <input
                ...
                className="w-full text-xl font-bold text-[#1F3649] bg-transparent border-none outline-none placeholder:text-[#adb3b4] placeholder:font-bold"
              />
            ...
            <div className="flex items-center justify-end gap-2 px-7 py-4 border-t border-[#f2f4f4] shrink-0">
              <Dialog.Close asChild>
                <button className="text-sm font-semibold text-[#5a6061] hover:text-[#1F3649] px-4 py-2 rounded-[10px] hover:bg-[#f2f4f4] transition-all cursor-pointer">
                  Cancel
                </button>
              </Dialog.Close>
              <button
